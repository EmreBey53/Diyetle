// src/screens/CreateDietPlanScreen.tsx
...
export default function CreateDietPlanScreen({ route, navigation }: any) {
  ...
  const [selectedWaterGoal, setSelectedWaterGoal] = useState(2.5);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (dailyCalorieTarget) {
        planData.dailyCalorieTarget = Number(dailyCalorieTarget);
      }

      if (dailyWaterGoal) {
        planData.dailyWaterGoal = Number(dailyWaterGoal);
      }
          ...
          {meals.length > 0 && (
            <View style={styles.summaryCard}>
              ...
              <Text style={styles.summaryText}>
                Toplam Öğün: {meals.length}
              </Text>
              <Text style={styles.summaryText}>
                Toplam Besin: {meals.reduce((sum, meal) => sum + meal.foods.length, 0)}
              </Text>
              {dailyCalorieTarget && (
